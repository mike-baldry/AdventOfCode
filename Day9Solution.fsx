...
let part1 () =
    ...
    let rec findInvalidNumbers (numbers : int64 list) (numberToConsider : int) (index : int) = 
        seq {
            let previousNumbers = 
                numbers.[index - numberToConsider..index]
            // Lazily get all possible sums of the previous X numbers
            let allSums =
                combinations [] 2 previousNumbers
                |> Seq.map (fun s -> s |> List.reduce (+))
                |> Seq.distinct
            let targetValue = numbers.[index]
            printfn $"Numbers to consider: {previousNumbers}, sums: %A{allSums}, target value: {targetValue}"
            if not (allSums |> Seq.contains targetValue) then
                yield targetValue
            else 
                yield! findInvalidNumbers numbers numberToConsider (index + 1)
        }


    findInvalidNumbers allNumbers 25 25
